...
ISSUE-001: a plan can cite a placement/ownership decision and then contradict it
in its file allocation. The canonical Planning flow therefore owns a hard-STOP
consistency gate plus an anti-bias prompt, scoped to placement/ownership only.
...
def _consistency_section(planning: str) -> str:
    """Return the File Allocation Consistency section body, scoped so token
    ...
    marker = "File Allocation Consistency — STOP Gate"
    start = planning.find(marker)
    assert start != -1, "planning flow has no file-allocation consistency gate"
...
def test_dc2_consistency_check_is_a_stop_gate_not_advisory() -> None:
    ...
    section = _consistency_section(read("skills/planning/flow.md"))
...
def test_dc4_anti_bias_prose_is_distinct_from_the_stop_gate() -> None:
    ...
    # cited placement-decision instruction — asserted on the directive line,
    # so moving the reference out of the anti-bias prose reddens the test.
    anti = anti_lines[0].lower()
    assert "derive" in anti
    assert "placement/ownership decisions" in anti
...
def test_stop_gate_precedes_task_decomposition_step() -> None:
    ...
    check_pos = planning.find("File Allocation Consistency — STOP Gate")
    decomp_pos = planning.find("## 4. Task Decomposition and Reference Graph")
    assert check_pos != -1
    assert decomp_pos != -1
